...
local function StartTravel()
    ...
    if not TargetCFrame or not Root or not Humanoid then
        ...
        task.delay(1.5, function()
            if TravelButton.Parent then TravelButton.Text = "IR PELO TRAJETO" end
        end)
        return
    end
    ...
    Traveling, CancelTravel = true, false
    TravelButton.Text = "CANCELAR TRAJETO"

    local Destination = TargetCFrame.Position
    local StartPosition = Root.Position
    local UndergroundY = math.min(StartPosition.Y, Destination.Y) - 25
    ...
    if UndergroundEnabled then
        if MoveTo(Vector3.new(StartPosition.X, UndergroundY, StartPosition.Z))
            and MoveTo(Vector3.new(Destination.X, UndergroundY, Destination.Z)) then
            MoveTo(Vector3.new(Destination.X, Destination.Y + 10, Destination.Z))
        end
    else
        MoveTo(Destination)
    end
    ...
    Traveling, CancelTravel = false, false
...
CreateFloatingFeatureButton("InstantPrompt", "PROMPT", function() return InstantPromptEnabled end,
    function(v) InstantPromptEnabled = v; PromptButton.Text = "  INSTANT PROMPT • " .. (v and "ON" or "OFF"); if v then UpdatePrompts() end end, 200)
CreateFloatingFeatureButton("PromptTP", "PROMPT TP", function() return PromptTeleportEnabled end,
...
local function CreateFloatingFeatureButton(Key, Name, GetState, SetState, DefaultY)
    if FloatingButtons[Key] then return FloatingButtons[Key] end

    local B = Instance.new("TextButton")
    B.Name = "Button_" .. Key
    B.Parent = Gui
    B.Size = UDim2.fromOffset(96, 30)
    B.Position = FloatingButtonPositions[Key] or UDim2.fromOffset(10, DefaultY or 200)
    ...
    UserInputService.InputChanged:Connect(function(Input)
        if dragging and (Input.UserInputType == Enum.UserInputType.MouseMovement
            ...
            B.Position = UDim2.fromOffset(startPos.X.Offset + D.X, startPos.Y.Offset + D.Y)
        end
    end)
    UserInputService.InputEnded:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1
...
local function RefreshFloatingButtons()
    local map = {
        InstantPrompt = InstantPromptEnabled,
        PromptTP = PromptTeleportEnabled,
        Underground = UndergroundEnabled,
        Drop = SnooDropEnabled,
        Speed = WalkEnabled,
        Jump = JumpEnabled,
        Noclip = NoclipEnabled,
        AntiRagdoll = AntiRagdollEnabled,
        InfinityJump = InfiniteJumpEnabled,
        AutoSteal = AutoStealEnabled
    }
    for Key, State in pairs(map) do
        if FloatingButtons[Key] then
            FloatingButtons[Key].Visible = FloatingButtonsEnabled and State
        end
    end
end
...
SelectPage("Buttons")
Section("BUTTONS")
Label("Ative uma opção abaixo para criar um botão pequeno na tela.")
Description("Os botões ficam arrastáveis. Toque neles para ligar/desligar a opção.")

local function AddButtonOption(Name, Key, GetState, SetState)
    local B = Button("  " .. Name .. " • " .. (GetState() and "ON" or "OFF"))
    B.MouseButton1Click:Connect(function()
        SetState(not GetState())
        B.Text = "  " .. Name .. " • " .. (GetState() and "ON" or "OFF")
...
AddButtonOption("INFINITY JUMP", "InfinityJump",
...
AddButtonOption("AUTO STEAL", "AutoSteal",
