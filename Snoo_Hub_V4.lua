...
local AnimationPriority = Enum.AnimationPriority.Movement
...
AnimationEnabled = LoadedConfig.AnimationEnabled == true
UndergroundTravelEnabled = LoadedConfig.UndergroundTravelEnabled ~= false
UndergroundDepth = ApplyLoadedNumber("UndergroundDepth", UndergroundDepth, 5, 500)
...
local function LoadAnimationTrack(Name, Id, Priority, Looped)
    if not Humanoid or not AnimationEnabled then return nil end
    ...
    Track.Priority = Priority or AnimationPriority
    Track.Looped = Looped == true
    return Track
end

local function RefreshCustomAnimations()
    StopAnimationTracks()
    if not AnimationEnabled or not Humanoid then return end

    local Idle = LoadAnimationTrack("Idle", AnimationIds.Idle, Enum.AnimationPriority.Idle, true)
    local Walk = LoadAnimationTrack("Walk", AnimationIds.Walk, Enum.AnimationPriority.Movement, true)
    local Run = LoadAnimationTrack("Run", AnimationIds.Run, Enum.AnimationPriority.Movement, true)
    local Jump = LoadAnimationTrack("Jump", AnimationIds.Jump, Enum.AnimationPriority.Action, false)
    local Fall = LoadAnimationTrack("Fall", AnimationIds.Fall, Enum.AnimationPriority.Action, true)

    AnimationTracks.Idle = Idle
    AnimationTracks.Walk = Walk
    AnimationTracks.Run = Run
    AnimationTracks.Jump = Jump
    AnimationTracks.Fall = Fall

    if Idle then Idle:Play(0.15, 1, 1) end

    Humanoid.StateChanged:Connect(function(_, NewState)
        if not AnimationEnabled then return end

        local function PlayOnly(Name, Speed)
            ...
            if Track then
                if not Track.IsPlaying then Track:Play(0.12, 1, Speed or 1) end
        ...
        elseif NewState == Enum.HumanoidStateType.Running
            ...
            if Speed > 0.05 then
                if Speed >= 80 then
                    PlayOnly("Run", math.clamp(Speed / 80, 0.75, 2))
                else
                    PlayOnly("Walk", math.clamp(Speed / 16, 0.5, 2))
                end
            else
                PlayOnly("Idle", 1)
            end
...
local AnimationPage = CreatePage("Animations")
...
SelectPage("Animations")
Section("ANIMAÇÕES")
Label("IDLE • ANDAR • CORRER • PULAR • QUEDA")
Description("Use os IDs abaixo para personalizar as animações. Deixe os padrões para usar as animações R15/R6 compatíveis.")
local IdleAnimationBox = TextBox(AnimationIds.Idle, "Animation ID do Idle")
local WalkAnimationBox = TextBox(AnimationIds.Walk, "Animation ID do Walk")
local RunAnimationBox = TextBox(AnimationIds.Run, "Animation ID do Run")
local JumpAnimationBox = TextBox(AnimationIds.Jump, "Animation ID do Jump")
local FallAnimationBox = TextBox(AnimationIds.Fall, "Animation ID do Fall")
local AnimationButton = Button("  ANIMAÇÕES • OFF")
local ApplyAnimationButton = Button("  APLICAR ANIMAÇÕES")
...
local function StartTravel()
    ...
    -- O trajeto passa abaixo do mapa e só sobe no final.
    local UndergroundY = math.clamp(
        math.min(StartPosition.Y, Destination.Y) - math.max(UndergroundDepth, 5),
        -50000, 50000
    )
    ...
    local function MoveRoute(Target)
    ...
    while Traveling and not CancelTravel do
        if MoveRoute(Vector3.new(Root.Position.X, UndergroundY, Root.Position.Z)) then break end
    end

    -- 2) Vai em linha reta por baixo da terra.
    while Traveling and not CancelTravel do
        if MoveRoute(Vector3.new(Destination.X, UndergroundY, Destination.Z)) then break end
    end

    -- 3) Sobe somente quando chegar ao X/Z do destino.
    while Traveling and not CancelTravel do
        if MoveRoute(Destination) then break end
    end
...
local function ReadAnimationIds()
    local function NormalizeId(Value, Fallback)
        local Number = tostring(Value or ""):match("%d+")
        if Number then return "rbxassetid://" .. Number end
        return Fallback
    end

    AnimationIds.Idle = NormalizeId(IdleAnimationBox.Text, AnimationIds.Idle)
    AnimationIds.Walk = NormalizeId(WalkAnimationBox.Text, AnimationIds.Walk)
    AnimationIds.Run = NormalizeId(RunAnimationBox.Text, AnimationIds.Run)
    AnimationIds.Jump = NormalizeId(JumpAnimationBox.Text, AnimationIds.Jump)
    AnimationIds.Fall = NormalizeId(FallAnimationBox.Text, AnimationIds.Fall)
end
...
AnimationButton.MouseButton1Click:Connect(function()
    ...
    AnimationButton.Text = "  ANIMAÇÕES • " .. (AnimationEnabled and "ON" or "OFF")
...
ApplyAnimationButton.MouseButton1Click:Connect(function()
    ...
    RefreshCustomAnimations()
    AnimationButton.Text = "  ANIMAÇÕES • ON"
    ApplyAnimationButton.Text = "  APLICADO!"
    task.delay(1.2, function()
        if ApplyAnimationButton.Parent then
            ApplyAnimationButton.Text = "  APLICAR ANIMAÇÕES"
        end
    end)
    if SaveConfig then SaveConfig() end
end)
...
FloatingTP.Size = UDim2.fromOffset(172, 112)
...
for _, ButtonObject in ipairs({SaveTPButton, TPButtonSmall, TravelButtonSmall}) do
...
IdleAnimationBox.Text = AnimationIds.Idle
WalkAnimationBox.Text = AnimationIds.Walk
RunAnimationBox.Text = AnimationIds.Run
JumpAnimationBox.Text = AnimationIds.Jump
FallAnimationBox.Text = AnimationIds.Fall
AnimationButton.Text = "  ANIMAÇÕES • " .. (AnimationEnabled and "ON" or "OFF")

if AutoStealEnabled then StartAutoSteal() end
if InfiniteJumpEnabled then StartInfiniteJump() end
if AnimationEnabled then RefreshCustomAnimations() end
