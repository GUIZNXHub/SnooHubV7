...
local Existing = PlayerGui:FindFirstChild("SnooHub6")
if Existing then
	Existing:Destroy()
end
...
TPBestButton.MouseButton1Click:Connect(function()
	local Target = ESPFindBestTargetReference and ESPFindBestTargetReference()
	if Target then
		CurrentBestTarget = Target
...
local ESPBestCacheAt = 0
local ESP_SCAN_INTERVAL = 1.0 -- procura o melhor no máximo 1x por segundo

local function ESPFindBestTarget(Force)
	local Now = os.clock()
	if not Force and ESPBestCache and Now - ESPBestCacheAt < ESP_SCAN_INTERVAL then
		if ESPBestCache.Object and ESPBestCache.Object.Parent and ESPBestCache.Part and ESPBestCache.Part.Parent then
			return ESPBestCache
		end
	end
	...
	-- Varre os slots apenas durante o scan. Nunca faça GetDescendants por frame.
	for _, Object in ipairs(Plots:GetDescendants()) do
		local IsSlot, SlotNumber = ESPIsSlot(Object)
		if IsSlot then
			local Name, Priority = ESPGetBrainrotName(Object)
			local Part = Name and ESPGetPart(Object)
			if Name and Priority and Part and Priority < BestPriority then
				BestPriority = Priority
				Best = {Name=Name, Priority=Priority, Part=Part, Slot=SlotNumber, Object=Object}
			end
			-- Normalmente o Brainrot está em um descendente do slot.
			for _, Candidate in ipairs(Object:GetDescendants()) do
				if Candidate:IsA("Model") or Candidate:IsA("BasePart") then
					local CName, CPriority = ESPGetBrainrotName(Candidate)
					local CPart = CName and ESPGetPart(Candidate)
					if CName and CPriority and CPart and CPriority < BestPriority then
						BestPriority = CPriority
						Best = {Name=CName, Priority=CPriority, Part=CPart, Slot=SlotNumber, Object=Candidate}
					end
				end
			end
		end
...
local function ESPUpdate()
	...
	local Target = BestLineEnabled and CurrentBestTarget or ESPCurrentTarget

	if not Target
		or not Target.Object
		or not Target.Object.Parent
		or not Target.Part
		or not Target.Part.Parent then

		ESPCurrentTarget = ESPFindBestTarget(true)
		CurrentBestTarget = ESPCurrentTarget
		Target = BestLineEnabled and CurrentBestTarget or ESPCurrentTarget
	end
	...
	local TargetPoint, TargetVisible = Camera:WorldToViewportPoint(TargetPart.Position)
	...
	local X1, Y1 = PlayerPoint.X, PlayerPoint.Y
	local X2, Y2 = TargetPoint.X, TargetPoint.Y

	local DX = X2 - X1
	local DY = Y2 - Y1
	local Length = math.sqrt(DX * DX + DY * DY)

	-- SOMENTE propriedades da GUI são alteradas aqui.
	ESPLine.Position = UDim2.fromOffset(X1, Y1)
	ESPLine.Size = UDim2.fromOffset(Length, 3)
	ESPLine.Rotation = math.deg(math.atan2(DY, DX))
	ESPLine.Visible = true

	ESPName.Position = UDim2.fromOffset(X2, Y2 - 8)
...
local function ESPSetEnabled(State)
	...
	ESPGui.Enabled = UltraESPEnabled
...
-- A página nova não remove nem altera as páginas Movement/Utility/Jump/Credits.
SelectPage("ESP")

Section("ESP ULTRA")
Description("Escolhe o melhor Brainrot DISPONÍVEL nos modelos 1-8 dentro de Plots.")
Description("A posição do Brainrot (1, 2, 3... 8) não muda a preferência.")
...
-- Atualização visual da linha sem tocar no personagem.
local ESPNextVisualUpdate = 0
RunService.Heartbeat:Connect(function()
	if not UltraESPEnabled and not BestLineEnabled then return end
	local Now = os.clock()
	if Now < ESPNextVisualUpdate then return end
	ESPNextVisualUpdate = Now + (1 / 30) -- máximo 30 atualizações visuais/s
	ESPUpdate()
end)
...
task.spawn(function()
	while Gui.Parent do
		...
		if UltraESPEnabled or BestLineEnabled then
			local Found = ESPFindBestTarget(true)
			if Found then
				CurrentBestTarget = Found
				ESPCurrentTarget = Found
			end
			if UltraESPEnabled then
				ESPUltraButton.Text = "  ESP ULTRA • ON"
			end
		end
	end
end)
...
print("SnooHubV11 carregado corretamente.")
