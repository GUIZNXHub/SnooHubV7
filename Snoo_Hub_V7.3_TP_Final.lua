...
local function StartTravel()
    ...
    local R = GetCurrentRoot()
    if not TargetCFrame or not R then
    ...
    local Speed = math.clamp(tonumber(TravelBox.Text) or TravelValue, 1, 250)
    TravelValue = Speed
    ...
    local function MoveTo(Target)
        while Traveling and not CancelTravel do
            local RootNow = GetCurrentRoot()
            if not RootNow then break end

            local Delta = Target - RootNow.Position
            local Distance = Delta.Magnitude

            if Distance <= 1 then
                RootNow.CFrame = CFrame.new(Target)
            ...
            RootNow.CFrame = CFrame.new(RootNow.Position + Delta.Unit * Step)
            RootNow.AssemblyLinearVelocity = Vector3.zero
            RootNow.AssemblyAngularVelocity = Vector3.zero
        end
        return false
    ...
    MoveTo(TargetCFrame.Position)

    -- Finaliza exatamente no local salvo e sem velocidade residual.
