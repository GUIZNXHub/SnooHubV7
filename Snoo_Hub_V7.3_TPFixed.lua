...
-- ==================== TP CONTROL • SPAM TP ====================
local SpamTPEnabled = SpamTPEnabled or false
local SpamTPThread = nil
...
local function SafeTeleportToSaved()
    local Target = GetSafeSavedCFrame()
    local CurrentRoot = GetCurrentRoot()
    if not Target or not CurrentRoot then return false end
...
local function StopSpamTP()
    SpamTPEnabled = false
    if SpamTPButton and SpamTPButton.Parent then
        SpamTPButton.Text = "  SPAM TP • OFF"
    end
end

local function StartSpamTP()
    if SpamTPEnabled then return end

    if not GetSafeSavedCFrame() then
        SpamTPButton.Text = "  SALVE UMA LOCALIZAÇÃO"
        task.delay(1.2, function()
            if SpamTPButton and SpamTPButton.Parent and not SpamTPEnabled then
                SpamTPButton.Text = "  SPAM TP • OFF"
            end
        end)
        return
    ...
    SpamTPThread = task.spawn(function()
...
local function StartTravel()
    ...
    if not TargetCFrame or not CurrentRoot or not CurrentHumanoid then
        TravelButton.Text = "SALVE UMA LOCALIZAÇÃO"
        task.delay(1.3, function()
            if TravelButton and TravelButton.Parent then
    ...
    local Speed = math.clamp(tonumber(TravelBox.Text) or TravelValue, 1, 1000)
    TravelValue = Speed
    ...
    local OldAutoRotate = CurrentHumanoid.AutoRotate
    local OldPlatformStand = CurrentHumanoid.PlatformStand

    CurrentHumanoid.AutoRotate = false
    CurrentHumanoid.PlatformStand = true

    -- Impede física residual durante todo o trajeto.
    ...
    local function MoveTo(Position)
        while Traveling and not CancelTravel do
            ...
            -- Apenas muda CFrame; não aplica impulso/velocity.
    ...
    if UndergroundEnabled then
        -- Fica 25 studs abaixo, cruza no eixo X/Z e sobe.
        ...
        if MoveTo(Down) and MoveTo(Across) then
            Success = MoveTo(Destination)
        end
    else
        Success = MoveTo(Destination)
    end

    -- Sempre termina exatamente na posição salva, sem lançar o personagem.
    local FinalRoot = GetCurrentRoot()
    if FinalRoot and not CancelTravel then
        FinalRoot.CFrame = TargetCFrame
        FinalRoot.AssemblyLinearVelocity = Vector3.zero
        FinalRoot.AssemblyAngularVelocity = Vector3.zero
    end

    local FinalHumanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
    if FinalHumanoid then
        FinalHumanoid.PlatformStand = OldPlatformStand
        FinalHumanoid.AutoRotate = OldAutoRotate
    end

    local LastRoot = GetCurrentRoot()
    if LastRoot then
        LastRoot.AssemblyLinearVelocity = Vector3.zero
        LastRoot.AssemblyAngularVelocity = Vector3.zero
    end
