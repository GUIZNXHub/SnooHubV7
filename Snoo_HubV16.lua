...
local SmoothTexturesEnabled = false
local SmoothTextureOriginals = {}
local SmoothTextureConnection = nil
local SmoothTexturesWasLoaded = false
...
-- ==================== SMOOTH TEXTURES ====================
-- Troca apenas o Material dos BaseParts e guarda o material original.
local function SmoothTexturesSetEnabled(State)
    State = State == true
    if State == SmoothTexturesEnabled then return end

    if State then
        SmoothTexturesEnabled = true
        table.clear(SmoothTextureOriginals)

        -- Faz a conversão em lotes para não congelar a execução em mapas grandes.
        task.spawn(function()
            local Count = 0
            for _, Object in ipairs(workspace:GetDescendants()) do
                if not SmoothTexturesEnabled then break end
                if Object:IsA("BasePart") then
                    SmoothTextureOriginals[Object] = Object.Material
        ...
        if SmoothTextureConnection then SmoothTextureConnection:Disconnect() end
        SmoothTextureConnection = workspace.DescendantAdded:Connect(function(Object)
            if SmoothTexturesEnabled and Object:IsA("BasePart") then
                SmoothTextureOriginals[Object] = Object.Material
                Object.Material = Enum.Material.SmoothPlastic
            end
        end)
    else
        SmoothTexturesEnabled = false
        if SmoothTextureConnection then
            SmoothTextureConnection:Disconnect()
            SmoothTextureConnection = nil
        end

        task.spawn(function()
            local Count = 0
            for Object, OriginalMaterial in pairs(SmoothTextureOriginals) do
                if Object and Object.Parent and Object:IsA("BasePart") then
                    pcall(function() Object.Material = OriginalMaterial end)
                end
                Count += 1
                if Count % 200 == 0 then
                    task.wait()
...
SmoothTexturesWasLoaded = LoadedConfig.SmoothTexturesEnabled == true
local Languages = {
...
local SmoothTexturesButton = Button("  TEXTURAS SMOOTH • " .. (SmoothTexturesEnabled and "ON" or "OFF"))
local SmoothTexturesHint = Description("Material liso. Ao desligar, restaura os materiais anteriores.")

SmoothTexturesButton.MouseButton1Click:Connect(function()
    SmoothTexturesSetEnabled(not SmoothTexturesEnabled)
    SmoothTexturesButton.Text = "  TEXTURAS SMOOTH • " .. (SmoothTexturesEnabled and "ON" or "OFF")
    SaveConfig()
end)
...
if SmoothTexturesWasLoaded then
    task.defer(function()
        SmoothTexturesSetEnabled(true)
        if SmoothTexturesButton and SmoothTexturesButton.Parent then
            SmoothTexturesButton.Text = "  TEXTURAS SMOOTH • ON"
        end
    end)
end
...
print("SnooHubV16 carregado corretamente.")
