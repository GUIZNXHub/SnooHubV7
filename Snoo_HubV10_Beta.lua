...
local function ESPFindBrainrotName(Object)
    ...
    for _, Key in ipairs({"BrainrotName", "DisplayName", "Name", "Brainrot", "PetName"}) do
        local Value = Object:GetAttribute(Key)
        if typeof(Value) == "string" then table.insert(Candidates, Value) end
        local Child = Object:FindFirstChild(Key, true)
        if Child then
            if Child:IsA("StringValue") then table.insert(Candidates, Child.Value) end
            if Child:IsA("TextLabel") or Child:IsA("TextButton") then table.insert(Candidates, Child.Text) end
        end
    end
    for _, Candidate in ipairs(Candidates) do
        local P = ESPGetPriority(Candidate)
        if P then
            local N = ESPNormalize(Candidate)
            return ESPAliases[N] and Candidate or Candidate, P
        end
    end
    return nil, nil
end
...
local function ESPMakeLine()
    ...
    Line.BackgroundColor3 = Color3.fromRGB(255, 40, 40)
...
local function ESPMakeText()
    ...
    Text.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
    ...
    Text.Size = UDim2.fromOffset(180, 22)
    Text.Visible = false
    Text.ZIndex = 1001
...
local function ESPFindTargets()
    local PlotsFolder = workspace:FindFirstChild("Plots")
    if not PlotsFolder then return {} end
    local Results = {}
    local Seen = {}
    for _, Plot in ipairs(PlotsFolder:GetDescendants()) do
        if Plot.Name:lower() == "plot" then
            for _, Object in ipairs(Plot:GetDescendants()) do
                if (Object:IsA("Model") or Object:IsA("BasePart")) and not Seen[Object] then
                    local Name, Priority = ESPFindBrainrotName(Object)
                    if Name and Priority then
                        Seen[Object] = true
                        table.insert(Results, {Object = Object, Name = Name, Priority = Priority})
                    end
                end
            end
        end
...
local function ESPRefresh()
    ...
    local Targets = ESPFindTargets()
    local Alive = {}
    for _, Target in ipairs(Targets) do
        Alive[Target.Object] = true
        local Data = ESPObjects[Target.Object]
        if not Data then
            Data = {Line = ESPMakeLine(), Text = ESPMakeText()}
            ESPObjects[Target.Object] = Data
        end
        Data.Name = Target.Name
        Data.Priority = Target.Priority
    end
    for Object, Data in pairs(ESPObjects) do
        if not Alive[Object] or not Object.Parent then
            if Data.Line then Data.Line:Destroy() end
            if Data.Text then Data.Text:Destroy() end
            ESPObjects[Object] = nil
        end
...
local function ESPUpdateVisuals()
    ...
    local Origin, OriginOnScreen = Camera:WorldToViewportPoint(MyRoot.Position)
    for Object, Data in pairs(ESPObjects) do
        local Position = ESPGetPosition(Object)
        if Position and Object.Parent then
            ...
            if Visible then
                ...
                Data.Line.Position = UDim2.fromOffset(X1, Y1)
                Data.Line.Size = UDim2.fromOffset(Length, Data.Priority <= 3 and 3 or 2)
                Data.Line.Rotation = math.deg(math.atan2(DY, DX))
                Data.Line.Visible = true
                Data.Line.BackgroundTransparency = Data.Priority <= 3 and 0 or 0.15
                Data.Text.Position = UDim2.fromOffset(X2, Y2 - 5)
                Data.Text.Text = Data.Name
                Data.Text.TextColor3 = Data.Priority <= 3 and Color3.fromRGB(255, 90, 90) or Color3.fromRGB(255, 255, 255)
...
local function SetUltraESP(State)
    ...
    if UltraESPEnabled then
        ESPRefresh()
        ESPConnections.Render = ESPConnections.Render or RunService.RenderStepped:Connect(ESPUpdateVisuals)
        ESPConnections.Scan = ESPConnections.Scan or task.spawn(function()
...
SelectPage("ESP")
Section("ULTRA ESP")
Description("Procura os Brainrots dentro de Plots/plots/plot/Plot e prioriza os melhores.")
...
FloatingTP.Size = UDim2.fromOffset(172, 180)
...
SpamTPV1ButtonSmall.Position = UDim2.fromOffset(6, 92)
...
SpamTPV2ButtonSmall.Position = UDim2.fromOffset(6, 126)
...
for _, ButtonObject in ipairs({SaveTPButton, TPButtonSmall, TravelButtonSmall, SpamTPV1ButtonSmall, SpamTPV2ButtonSmall}) do
